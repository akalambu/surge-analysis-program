...
Begin VB.Form frmGridAlignB 
   ...
   End
End
Attribute VB_Name = "frmGridAlignB"
...
Private Sub cmdOK_Click()
   ...
   For ii = 1 To (MSFlexGrid1.Rows - 1) Step 1
      If IsNumeric(MSFlexGrid1.TextMatrix(ii, 1)) And IsNumeric(MSFlexGrid1.TextMatrix(ii, 2)) _
      And IsNumeric(MSFlexGrid1.TextMatrix(ii, 3)) Then
     PN = MSFlexGrid1.TextMatrix(ii, 1)
     NALIGN(PN) = NALIGN(PN) + 1
     CHAIN(PN, NALIGN(PN)) = MSFlexGrid1.TextMatrix(ii, 2)
     GL(PN, NALIGN(PN)) = MSFlexGrid1.TextMatrix(ii, 3)
     End If
   Next
   Unload Me
End Sub
...
Private Sub Form_Load()
   ...
   Left = 20
   Top = 30
   myArray = Array("Sl. No.", "Pipe No.", "Chainage      (m)", "Elevation    (RL, m)")
   NALIGNT = 0
   For ii = 1 To NPMAX
    NALIGNT = NALIGNT + NALIGN(ii)
   Next
   If NALIGNT > 0 Then
    MSFlexGrid1.Rows = NALIGNT + 1
   Else
    MSFlexGrid1.Rows = 2
   End If
   ...
   Next
   If NALIGNT > 0 And Not OpenFile = "" Then
   jj = 0
   For i = 1 To NPMAX
     For j = 1 To NALIGN(i)
       jj = jj + 1
       MSFlexGrid1.TextMatrix(jj, 1) = i
       MSFlexGrid1.TextMatrix(jj, 2) = CHAIN(i, j)
       MSFlexGrid1.TextMatrix(jj, 3) = GL(i, j)
     Next
   Next
   End If
