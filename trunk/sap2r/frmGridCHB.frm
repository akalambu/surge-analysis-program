...
Begin VB.Form frmGridCHB 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Data for Printing Heads"
   ClientHeight    =   4815
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4410
   ...
   Begin VB.Label Label1 
      Alignment       =   2  'Center
      Caption         =   "Enter Pipe No. and Chainages (m) for Printing Heads"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
...
NPRNT = 0
   For tst = 1 To (MSFlexGrid1.Rows - 1) Step 1
      If IsNumeric(MSFlexGrid1.TextMatrix(tst, 1)) And IsNumeric(MSFlexGrid1.TextMatrix(tst, 2)) Then
         NPRNT = NPRNT + 1
         NPPRNT(NPRNT) = MSFlexGrid1.TextMatrix(tst, 1)
         CHPRNT(NPRNT) = MSFlexGrid1.TextMatrix(tst, 2)
         
        End If
   Next

  For tst = 1 To (NPRNT - 1)
    For tst1 = tst + 1 To (NPRNT)
    If NPPRNT(tst) > NPPRNT(tst1) Then
       temp1 = NPPRNT(tst)
       temp2 = CHPRNT(tst)
       NPPRNT(tst) = NPPRNT(tst1)
       NPPRNT(tst1) = temp1
       CHPRNT(tst) = CHPRNT(tst1)
       CHPRNT(tst1) = temp2
    End If
    Next
  Next
 ...
 For tst = 1 To (NPRNT - 1)
    For tst1 = tst + 1 To (NPRNT)
    If (NPPRNT(tst) = NPPRNT(tst1)) Then
    If CHPRNT(tst) > CHPRNT(tst1) Then
       temp1 = NPPRNT(tst)
       temp2 = CHPRNT(tst)
       NPPRNT(tst) = NPPRNT(tst1)
       NPPRNT(tst1) = temp1
...
If NPRNT > 0 Then
  MSFlexGrid1.Rows = NPRNT + 1
   For i = 1 To MSFlexGrid1.Rows - 1
       MSFlexGrid1.TextMatrix(i, 1) = NPPRNT(i)
       MSFlexGrid1.TextMatrix(i, 2) = CHPRNT(i)
     Next
   End If
End Sub
Private Sub Form_Load()
  ...
  'set array
   myArray = Array("Sl. No.", "Pipe No.", "Chainage (m)")
