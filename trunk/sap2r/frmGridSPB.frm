...
Begin VB.Form frmGridSPB 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Data for Stand Pipes"
   ClientHeight    =   3870
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   7950
   ...
   Begin VB.Frame frmINRVs 
      Caption         =   "Enter the Data for Stand Pipes"
      BeginProperty Font 
         Name            =   "Comic Sans MS"
         Size            =   12
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmGridSPB"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Const TOTALCOLUMNS = 6  'Zero based
...
Private Sub Form_Load()
   ...
   Top = 30
    ...
    myArray = Array("Stand Pipe No.", "Pipe No.", "Chainage   (m)", "Diameter (mm)", "Pipe Invert Level (RL, m)", "Top Level (RL, m)")
   
   If NSSD > 0 Then
    MSFlexGrid1.Rows = NSSD + 1
    txtSP = NSSD
   Else
    MSFlexGrid1.Rows = 2
   End If
   ...
   Next
   If NSSD > 0 And Not OpenFile = "" Then
   For i = 1 To MSFlexGrid1.Rows - 1
     For j = 1 To MSFlexGrid1.Cols - 1
       MSFlexGrid1.TextMatrix(i, j) = SP_Data(i - 1, j - 1)
     Next
   Next
   End If
   'highlight 1st row
