...
Begin VB.Form frmGridSOV 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Source Reservoir Details"
   ClientHeight    =   3570
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4830
   ...
   Begin VB.Frame frmINRVs 
      Caption         =   "Enter the Source Reservoir Details"
      BeginProperty Font 
         Name            =   "Comic Sans MS"
         Size            =   12
         Charset         =   0
...
Dim jj As Integer
 ...
 For ii = 1 To NSOU
   SOUWL(ii) = MSFlexGrid1.TextMatrix(ii, 2)
   SOUDC(ii) = MSFlexGrid1.TextMatrix(ii, 3)
   ISOU(MSFlexGrid1.TextMatrix(ii, 1)) = ii
   NPDSSO(ii) = IDSP(MSFlexGrid1.TextMatrix(ii, 1), 1)
    If Abs(SOUDC(ii) - PDC(IDSP(MSFlexGrid1.TextMatrix(ii, 1), 1))) > 0.002 Then
    MsgBox "Reservoir outflow does not match approach pipe flow for reservoir :" & ii
    Exit Sub
   End If
 Next
Me.Hide
...
Private Sub Form_Load()
   ...
   myArray = Array("Sl. No.", "Node No.", "Water Level (RL, m)", "Discharge (cum/sec)")
   Left = 20
   Top = 30
   ...
   MSFlexGrid1.RowHeight(0) = 600
      ...
      MSFlexGrid1.ColWidth(iCount) = 650
   ...
   For ii = 1 To MSFlexGrid1.Rows - 1
       MSFlexGrid1.TextMatrix(ii, 2) = SOUWL(ii)
       MSFlexGrid1.TextMatrix(ii, 3) = SOUDC(ii)
   Next
  End If
   'highlight 1st row
   HighLightGridRow (1)
