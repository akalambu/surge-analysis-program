...
Begin VB.Form frmGridInB 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Data for INRV"
   ClientHeight    =   3825
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6705
   ...
   Begin VB.Frame frmINRVs 
      Caption         =   "Enter the Data for INRV"
      BeginProperty Font 
         Name            =   "Comic Sans MS"
         Size            =   12
         Charset         =   0
   ...
   Begin VB.Label Label1 
      Caption         =   "Number of Intermediate Non-Return Valves"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
...
Dim flag As Integer
flag = 0
NNRVD = Val(txtIN.Text)

If NNRVD > 0 Then
  For ii = 0 To (NNRVD - 1) Step 1
     For jj = 0 To 3 Step 1
        If Not IsNumeric(MSFlexGrid1.TextMatrix(ii + 1, jj + 1)) Then
         ...
         Exit For
         End If
        INR_Data(ii, jj) = MSFlexGrid1.TextMatrix(ii + 1, jj + 1)
        Next
     If flag = 1 Then
      Exit For
     End If
...
Private Sub Form_Load()
   ...
   'set array
    myArray = Array("INRV No.", "Pipe No.", "Chainage   (m)", "Bypass Size (mm)", "Delay in Closure (sec)")
  
   If NNRVD > 0 Then
    MSFlexGrid1.Rows = NNRVD + 1
    txtIN = NNRVD
   Else
    MSFlexGrid1.Rows = 2
   End If
   ...
   If NNRVD > 0 And Not OpenFile = "" Then
   For i = 1 To MSFlexGrid1.Rows - 1
     For j = 1 To MSFlexGrid1.Cols - 1
       MSFlexGrid1.TextMatrix(i, j) = INR_Data(i - 1, j - 1)
     Next
   Next
   End If
   'highlight 1st row
