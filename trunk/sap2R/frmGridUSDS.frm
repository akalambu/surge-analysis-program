...
Begin VB.Form frmGridUSDS 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Information of Upstream or Downstream pipes"
   ClientHeight    =   3750
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5760
...
flag = 0
         For jj = 1 To TempNUD
         If Not IsNumeric(MSFlexGrid1.TextMatrix(1, jj)) Then
         MsgBox "Data is not Complete, Please Check It !!"
         flag = 1
         Exit For
         End If
         USDS(jj) = MSFlexGrid1.TextMatrix(1, jj)
        Next
     If flag = 1 Then
      Exit For
     End If
...
Private Sub Form_Load()
   ...
   For j = 1 To MSFlexGrid1.Rows - 1
       If Not USDS(j) = 0 Then
       MSFlexGrid1.TextMatrix(1, j) = USDS(j)
     Next
   Next
   End If
