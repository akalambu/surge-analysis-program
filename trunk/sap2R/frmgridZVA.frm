...
Begin VB.Form frmGridZVA 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Data for ZVV"
   ClientHeight    =   4125
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5550
   ...
   Begin VB.TextBox txtZV 
      ...
      Width           =   855
   End
   Begin VB.Frame frmZV 
      Caption         =   "Enter the Data for ZVV"
      BeginProperty Font 
         Name            =   "Comic Sans MS"
         Size            =   12
         Charset         =   0
...
If NZVD > 0 Then
   For ii = 0 To (NZVD - 1) Step 1
    ...
    Next
Else
frmProtA.chkZeroV.Value = 0
frmProtA.chkDualPl.Enabled = True
End If
If flag = 0 Then
 Me.Hide
...
Private Sub Form_Load()
   ...
   Dim iCount As Integer
    Left = 20
    Top = 30
    myArray = Array("ZVV No.", "Chainage   (m)", "Bypass Size (mm)")
   
   If NZVD > 0 Then
    MSFlexGrid1.Rows = NZVD + 1
    txtZV = NZVD
   Else
    MSFlexGrid1.Rows = 2
   End If
   ...
   Next
   If NZVD > 0 And Not OpenFile = "" Then
   For i = 1 To MSFlexGrid1.Rows - 1
     For j = 1 To MSFlexGrid1.Cols - 1
       MSFlexGrid1.TextMatrix(i, j) = ZV_Data(i - 1, j)
     Next
   Next
   End If
   'highlight 1st row
...
' Use the character that was typed.
   ...
   Edt.SetFocus
   End Sub
Private Sub txtZV_LostFocus()
If Val(txtZV.Text) = 0 Then
 MSFlexGrid1.Rows = 2
...
Else
  ...
  MSFlexGrid1.Rows = txtZV.Text + 1
  For i = 1 To MSFlexGrid1.Rows - 1
    MSFlexGrid1.TextMatrix(i, 0) = i
  Next
End If
End Sub
Private Sub txtZV_GotFocus()
MSFlexGrid1.Enabled = True
End Sub
