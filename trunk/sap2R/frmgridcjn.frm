...
Begin VB.Form frmGridCJN 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Combining Junction Details"
   ClientHeight    =   3750
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4830
   ...
   Begin VB.Frame frmINRVs 
      Caption         =   "Enter the Combining Junction Details"
      BeginProperty Font 
         Name            =   "Comic Sans MS"
         Size            =   12
         Charset         =   0
...
Dim jj As Integer
  ...
  Next
 For ii = 1 To NCJN
   HGL(MSFlexGrid1.TextMatrix(ii, 1)) = MSFlexGrid1.TextMatrix(ii, 2)
 Next
Me.Hide
frmHGL.Enabled = True
...
Private Sub Form_Load()
   ...
   'set array
    myArray = Array("Sl. No.", "Node No.", "HGL (RL, m)")
  ...
  If Not OpenFile = "" Then
   For ii = 1 To MSFlexGrid1.Rows - 1
       MSFlexGrid1.TextMatrix(ii, 2) = HGL(MSFlexGrid1.TextMatrix(ii, 1))
   Next
  End If
   'highlight 1st row
   HighLightGridRow (1)
