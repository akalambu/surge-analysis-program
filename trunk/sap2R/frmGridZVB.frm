  ...
  For ii = 0 To (NZVD - 1) Step 1
     For jj = 0 To 2 Step 1
        If Not IsNumeric(MSFlexGrid1.TextMatrix(ii + 1, jj + 1)) Then
         ...
         Exit For
         End If
        ZV_Data(ii, jj) = MSFlexGrid1.TextMatrix(ii + 1, jj + 1)
        Next
     If flag = 1 Then
      Exit For
     End If
    Next
Else
frmProtB.chkZeroV.Value = 0
frmProtB.chkDualPl.Enabled = True
End If
If flag = 0 Then
 Me.Hide
...
Private Sub Form_Load()
   ...
   Dim iCount As Integer
    ...
    Left = 20
    Top = 30
    myArray = Array("ZVV No.", "Pipe No.", "Chainage   (m)", "Bypass Size (mm)")
   If NZVD > 0 Then
   ...
   For i = 1 To MSFlexGrid1.Rows - 1
     For j = 1 To MSFlexGrid1.Cols - 1
       MSFlexGrid1.TextMatrix(i, j) = ZV_Data(i - 1, j - 1)
     Next
   Next
   End If
   'highlight 1st row
