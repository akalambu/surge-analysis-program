...
Begin VB.Form frmGridPumpCH 
   ...
   End
End
Attribute VB_Name = "frmGridPumpCH"
...
NPUMPCH(NP) = 0
   For ii = 1 To (MSFlexGrid1.Rows - 1) Step 1
    ...
    For ii = 1 To NPUMPCH(NP)
       DCP(NP, ii) = MSFlexGrid1.TextMatrix(ii, 1)
       HEADP(NP, ii) = MSFlexGrid1.TextMatrix(ii, 2)
       EFFP(NP, ii) = MSFlexGrid1.TextMatrix(ii, 3)
    Next
...
Private Sub Form_Load()
   ...
   If NPUMPCH(NP) > 0 Then
    MSFlexGrid1.Rows = NPUMPCH(NP) + 1
   Else
    MSFlexGrid1.Rows = 2
   End If
   ...
   Next
'   If NPUMPCH(NP) > 0 And Not OpenFile = "" Then
'     For i = 1 To MSFlexGrid1.Rows - 1
'       MSFlexGrid1.TextMatrix(i, 1) = DCP(NP, i)
'       MSFlexGrid1.TextMatrix(i, 2) = HEADP(NP, i)
'       MSFlexGrid1.TextMatrix(i, 3) = EFFP(NP, i)
'     Next
