     ...
     * ICDS(80),IOBS(80),IPMP(80),IBST(80),HGL(80),RESWL(80),
     * RESDC(80),SOUWL(5),SOUDC(5),HGL1CD(10),HGL2CD(10),QOCDS(10),
     * HGL1OB(9),HGL2OB(9),QOOBS(9),NPUMPS(10),PUMPDC(10),QRPMP(10),
     ...
     * NPUSCD(10),NPUSOB(9),NPUSRS(80),NPDSSO(5),CHST(80),CHSV(10),
     * NALIGN(80),CHAIN(80,400),GL(80,400),NPPRNT(500),IPRNT(500),
     * CHPRNT(500),ITRIP(10),ITRIPB(5),EFFP(10),EFFB(5),
     * HLCDS(10),HLOBS(5),NNPRNT(500),NPCS(5),CHCS(5),PLCS(5),
     ...
     * TVSVD(10),NPPATH(7),IPPATH(7,80),KODEDV(10),NSN(10),CHDEV(10),
     * ntemp1(500),temp2(500)
     ...
     * VUMIN(600),VDMIN(600),VUMAX(600),VDMAX(600),HGLR(80),
     ...
     * TAUZV(21),IZV(10),TAZV(10),AR(80),HRATB(5),CIB(5),
     * HRAT(10),PUMPK(10),CI(10),DHOB(5),DHO(10),BSTK(5),VPUIB(5),
...
C READ THE INPUT DATA - FROM VB DUMP
     ...
     *            IBST(I),HGL(I)
	 if(type(i).ne.'RES')ires(i)=0
	 if(type(i).ne.'SOU')isou(i)=0
	 if(type(i).ne.'CDS')icds(i)=0
	 if(type(i).ne.'OBS')iobs(i)=0
	 if(type(i).ne.'PMP')ipmp(i)=0
	 if(type(i).ne.'BST')ibst(i)=0
	END DO
	...
	nalign(2)=nalign(1)
	do kk=1,nalign(2)
	chain(2,kk)=chain(1,kk)
	gl(2,kk)=gl(1,kk)
	end do
	nalign(1)=2
	chain(1,1)=chst(1)
	chain(1,2)=chst(1)+plen(1)
	gl(1,2)=gl(1,1)
	nppath(1)=2
	ippath(1,1)=1
	ippath(1,2)=2
	...
	chprnt(1)=chst(1)
	do jj=2,nprnt
	npprnt(jj)=ntemp1(jj-1)
	chprnt(jj)=temp2(jj-1)
	end do
	endif	
     ...
     *            PHISV(I),CTIMSV(I)
	 END DO
	 if(siml.eq.'TYPEA'.and.npmax.eq.1)npsv(1)=1	 
	END IF
	READ(1,*)CODIV
...
142 	FORMAT(/3X,'SL NO.',2X,'LOCATION',2X,'GROUND LEVEL',2X,
     ...
     *  YSTOST(I),DCOST(I)
143	FORMAT(5X,I2,5X,F7.0,5X,F8.3,6X,F5.2,4X,F7.3,3X,F5.2,7X,F5.0)
...
145 	FORMAT(/2X,'SL NO',1X,'PIPE NO',2X,'LOCATION',4X,
     ...
     *  YSTOST(I),DCOST(I)
150	FORMAT(3X,I2,4X,I3,5X,F7.0,4X,F7.2,2X,F5.2,2X,F7.3,3X,
     *  F5.2,7X,F5.0)
	DCOST(I)=0.001*DCOST(I)
	END DO
	ENDIF
...
168	FORMAT(4X,I3,5X,F7.0,6X,F5.0)
...
175	FORMAT(4X,I3,6X,I3,5X,F7.0,6X,F5.0)
	DBYCV(I)=0.001*DBYCV(I)
	END DO
	ENDIF
	ENDIF
...
208	FORMAT(4X,I3,5X,F7.0,6X,F5.0,4X,F7.2)
...
215	FORMAT(4X,I3,6X,I3,5X,F7.0,6X,F5.0,4X,F7.2)
	DBYNRV(I)=0.001*DBYNRV(I)
	END DO
	ENDIF
	ENDIF
...
228	FORMAT(3X,I3,4X,F4.0,3X,F7.0,4X,F8.3)
	END DO
	ELSE
	WRITE (5,230) 
...
235	FORMAT(3X,I3,4X,F4.0,5X,I3,4X,F7.0,4X,F8.3)
	END DO
	ENDIF
	ENDIF
...
258	FORMAT(4X,I3,6X,F7.0,5X,F5.0,4X,F8.3,7X,F8.3)
...
265	FORMAT(4X,I3,6X,I3,6X,F7.0,5X,F5.0,4X,F8.3,5X,F8.3)
	SSDIA(I)=0.001*SSDIA(I)
	END DO
	ENDIF
	ENDIF
...
285	FORMAT(/2X,
     ...
     *  'CHAINAGE OF INTERMEDIATE VALVE          =',F7.0/2X,
...
1130	FORMAT(2X,I3,2X,I3,2X,F7.5,2X,F7.5)
	...
	ENDIF
	IF(CODIV.EQ.'YES')THEN
	WRITE(6,1135)KIIV,TAIV,TABIV,VBYIV
1135	FORMAT(/2X,'KIIV=',I3,2X,'TAIV=',F6.4,2X,'TABIV=',F6.4,2X,
     *  'VBYIV=',F8.5)
	ENDIF
...
1150    FORMAT(2X,I3,3X,I3,4F12.5)
     ...
     *    VPUB(IB),WHB,DHOB(IB),CM,CP,B(NPR),B(NPL),HNUGN,HNDGN,
...
2840	FORMAT(/2X,'MINIMUM & MAXIMUM PIEZOMETRIC HEADS AT ' 
	...
	IF(SIML.EQ.'TYPEA')THEN
	WRITE(5,2845)
2845	FORMAT(10X,'SPECIFIED',3X,'NEAREST NODAL',2X,'MIN HEAD',2X,
     *  'MAX HEAD'/9X,'CHAINAGE(m)',3X,'CHAINAGE(m)',4X,'(RL,m)',4X,
     *  '(RL,m)'/)
	DO I=1,NPRNT
	WRITE(5,2850) CHPRNT(I),CHNODE(I),HMIND(I),HMAXD(I)
2850	FORMAT(11X,F8.1,6X,F8.1,5X,F7.2,4X,F7.2)
	END DO
	ELSE
	WRITE(5,2860)
...
2870	FORMAT(11X,I3,4X,F8.1,6X,F8.1,5X,F7.2,4X,F7.2)
	END DO
	ENDIF
