...
Begin VB.Form frmgridOWSTB 
   ...
   End
End
Attribute VB_Name = "frmgridOWSTB"
   ...
   For ii = 0 To (NOSTD - 1) Step 1
     For jj = 0 To 6 Step 1
        If Not IsNumeric(MSFlexGrid1.TextMatrix(ii + 1, jj + 1)) Then
         ...
         Exit For
         End If
        OST_Data(ii, jj) = MSFlexGrid1.TextMatrix(ii + 1, jj + 1)
     Next
     If flag = 1 Then
      Exit For
     End If
...
Private Sub Form_Load()
   ...
   Top = 30
    ...
    myArray = Array("OST No.", "Pipe No.", "Chainage   (m)", "Ground Level (RL, m)", "Staging Height (m)", "Diameter   (m)", "Storage Depth (m)", "Connecting Pipe Dia (mm)")
   ...
   For i = 1 To MSFlexGrid1.Rows - 1
     For j = 1 To MSFlexGrid1.Cols - 1
       MSFlexGrid1.TextMatrix(i, j) = OST_Data(i - 1, j - 1)
     Next
   Next
   End If
   'highlight 1st row
