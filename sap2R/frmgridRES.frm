...
Begin VB.Form frmGridRES 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Delivery Reservoir Details"
   ClientHeight    =   3750
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5445
   ...
   Begin VB.Frame frmINRVs 
      Caption         =   "Enter the Delivery Reservoir Details"
      BeginProperty Font 
         Name            =   "Comic Sans MS"
         Size            =   12
         Charset         =   0
...
Dim jj As Integer
 ...
 For ii = 1 To NRES
   RESWL(ii) = MSFlexGrid1.TextMatrix(ii, 2)
   RESDC(ii) = MSFlexGrid1.TextMatrix(ii, 3)
   NPUSRS(ii) = IUSP(MSFlexGrid1.TextMatrix(ii, 1), 1)
   IRES(MSFlexGrid1.TextMatrix(ii, 1)) = ii
    If Abs(RESDC(ii) - PDC(IUSP(MSFlexGrid1.TextMatrix(ii, 1), 1))) > 0.002 Then
    MsgBox "Reservoir inflow does not match approach pipe flow for reservoir :" & ii
    Exit Sub
   End If
 Next
Me.Hide
...
Private Sub Form_Load()
  ...
  If Not OpenFile = "" Then
   For ii = 1 To MSFlexGrid1.Rows - 1
       MSFlexGrid1.TextMatrix(ii, 2) = RESWL(ii)
       MSFlexGrid1.TextMatrix(ii, 3) = RESDC(ii)
   Next
  End If
  'highlight 1st row
   HighLightGridRow (1)
