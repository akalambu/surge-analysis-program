c	look for line 326 for npsv correction for typea
     ...
     * NPUSCD(5),NPUSOB(5),NPUSRS(50),NPDSSO(5),CHST(5),CHSV(10),
	...
	OPEN(UNIT=4,FILE='KS.S11')
	OPEN(UNIT=5,FILE='SAP2.RES')
	OPEN(UNIT=6,FILE='F6.RES')
	OPEN(UNIT=8,FILE='TABLE.RES')

	OPEN(UNIT=7,FILE='RATE.DAT')
	OPEN(UNIT=11,FILE='VOST1.DAT')
	OPEN(UNIT=21,FILE='OST1.DAT')
	OPEN(UNIT=12,FILE='VOST2.DAT')
	OPEN(UNIT=22,FILE='OST2.DAT')
	OPEN(UNIT=13,FILE='VOST3.DAT')
	OPEN(UNIT=23,FILE='OST3.DAT')
	OPEN(UNIT=14,FILE='VOST4.DAT')
	OPEN(UNIT=24,FILE='OST4.DAT')
	OPEN(UNIT=10,FILE='SPEED.DAT')
	OPEN(UNIT=20,FILE='RPM.DAT')
	OPEN(UNIT=15,FILE='VAV.DAT')
	OPEN(UNIT=25,FILE='AVS.DAT')
	OPEN(UNIT=26,FILE='PLOT1.DAT')
	OPEN(UNIT=27,FILE='PLOT2.DAT')
	OPEN(UNIT=28,FILE='PLOT3.DAT')
	OPEN(UNIT=29,FILE='PLOT4.DAT')
	OPEN(UNIT=30,FILE='PLOT5.DAT')
	OPEN(UNIT=31,FILE='PLOT6.DAT')
...
9	FORMAT(/2X,'TRANSMISSION MAIN DETAILS:')
	DO II=1,NPMAX
	IF(NPMAX.GT.1.AND.SIML.EQ.'TYPEB')THEN
...
30	FORMAT(/2X,
	...
	IF(CODPMP(II).EQ.'YES')THEN
	 WRITE(5,35)NPUMPS(II),PUMPDC(II),QRPMP(II),PUMPH(II),
...
35	 FORMAT(2X,
	...
	ELSE
	 WRITE(5,40)NPUMPS(II),PUMPDC(II),PUMPH(II),IPMPSP(II),
     *   SUMPWL(II),KODPHV(II)
40	 FORMAT(2X,
     * 	 'NUMBER OF WORKING PUMPS                 =',I3/2X,
...
57	FORMAT(2X,
	...
	ENDIF
	IF(NPMP.EQ.1.AND.(SIML.EQ.'SPF'.OR.SIML.EQ.'APF'))THEN
	WRITE(5,60)PDIA(1),WV(1),SIML
60	FORMAT(2X,
     *  'PUMP DELIVERY PIPE DIAMETER (mm)        ='F5.0/2X,
     *  'PRESSURE WAVE VELOCITY IN DELIVERY PIPE ='F6.0/2X,
     *  '                                ( m/sec)'/2X,
     *   'TYPE OF ANALYSIS                        =',1X,A3)
	ENDIF
	END DO
	ENDIF
...
100	FORMAT(2X,
     ...
     *  'RESERVOIR WATER LEVEL OR DELIVERY LEVEL =',F7.2
     *  '                                [RL,m]')     
	END DO
...
257	FORMAT(3X,'SL NO.',3X,'LOCATION'2X,'DIAMETER'2X,'PIPE INVERT',2X,
     *  'TOP LEVEL'/11X,'Chainage,m',4X,'mm',6X,'LEVEL RL,m',2X,
     *  'OF PIPE RL,m'/) 
	DO I=1,NSSD
	WRITE (5,258) I,CHSS(I),SSDIA(I),SSINV(I),SSTOP(I)
258	FORMAT(4X,I3,6X,F6.0,5X,F5.0,4X,F8.3,4X,F8.3)
