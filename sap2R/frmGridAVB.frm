  ...
  For ii = 0 To (NAVD - 1) Step 1
     For jj = 0 To 3 Step 1
        If Not IsNumeric(MSFlexGrid1.TextMatrix(ii + 1, jj + 1)) Then
         ...
         Exit For
         End If
        AVV_Data(ii, jj) = MSFlexGrid1.TextMatrix(ii + 1, jj + 1)
        Next
     If flag = 1 Then
      Exit For
     End If
...
Private Sub Form_Load()
   ...
   Top = 30
    ...
    myArray = Array("AV No.", "Pipe No.", "Size  (mm)", "Chainage   (m)", "Pipe Invert Level (RL, m)")
   ...
   For i = 1 To MSFlexGrid1.Rows - 1
     For j = 1 To MSFlexGrid1.Cols - 1
       MSFlexGrid1.TextMatrix(i, j) = AVV_Data(i - 1, j - 1)
     Next
   Next
   End If
   'highlight 1st row
