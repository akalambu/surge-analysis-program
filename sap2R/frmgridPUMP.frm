...
Begin VB.Form frmGridPUMP 
   ...
   Begin VB.Frame frmINRVs 
      Caption         =   "Enter the Pump Details"
      BeginProperty Font 
         Name            =   "Comic Sans MS"
         Size            =   12
         Charset         =   0
      ...
      Begin VB.ComboBox Combo1 
         ...
         EndProperty
         Height          =   360
         ItemData        =   "frmgridPUMP.frx":0000
         Left            =   7440
         List            =   "frmgridPUMP.frx":0002
         TabIndex        =   4
         Top             =   960
         Visible         =   0   'False
         Width           =   975
...
For ii = 1 To NPMP
 ...
 For ii = 1 To NPMP
   NPUMPS(ii) = MSFlexGrid1.TextMatrix(ii, 2)
   PUMPDC(ii) = MSFlexGrid1.TextMatrix(ii, 3)
   PUMPH(ii) = MSFlexGrid1.TextMatrix(ii, 4)
   PUMPSP(ii) = MSFlexGrid1.TextMatrix(ii, 5)
   SUMPWL(ii) = MSFlexGrid1.TextMatrix(ii, 6)
   KODPHV(ii) = MSFlexGrid1.TextMatrix(ii, 7)
   CODPMP(ii) = MSFlexGrid1.TextMatrix(ii, 8)
   IPMP(MSFlexGrid1.TextMatrix(ii, 1)) = ii
   NPDSPM(ii) = IDSP(MSFlexGrid1.TextMatrix(ii, 1), 1)
   
  If Abs(NPUMPS(ii) * PUMPDC(ii) - PDC(IDSP(MSFlexGrid1.TextMatrix(ii, 1), 1))) > 0.002 Then
   MsgBox "Mismatch between individual pump discharge and total discharge for operating pump : " & ii
   Exit Sub
  End If
 Next
...
Private Sub Combo1_Click()
     ...
     If Combo1.Text = "YES" Then
       frmGridPUMP.Enabled = False
       frmPMachY.Show
      ElseIf Combo1.Text = "NO" Then
       PUMPSP(NP) = MSFlexGrid1.TextMatrix(NP, 5)
       PUMPDC(NP) = MSFlexGrid1.TextMatrix(NP, 3)
       PUMPH(NP) = MSFlexGrid1.TextMatrix(NP, 4)
       frmGridPUMP.Enabled = False
       frmPMachN.Show
     End If
End Sub
...
Private Sub Form_Load()
   ...
   Combo1.Visible = False
    ...
    myArray = Array("Sl. No.", "Node No.", "No. of Pumps", "Discharge (cum/sec)", "Head (m)", "Speed (rpm)", "Sump Water Level (RL, m)", "Type of NRV", "Machinery Finalised ?")
  ...
  If Not OpenFile = "" Then
   For ii = 1 To MSFlexGrid1.Rows - 1
     MSFlexGrid1.TextMatrix(ii, 2) = NPUMPS(ii)
     MSFlexGrid1.TextMatrix(ii, 3) = PUMPDC(ii)
     MSFlexGrid1.TextMatrix(ii, 4) = PUMPH(ii)
     MSFlexGrid1.TextMatrix(ii, 5) = PUMPSP(ii)
     MSFlexGrid1.TextMatrix(ii, 6) = SUMPWL(ii)
     MSFlexGrid1.TextMatrix(ii, 7) = KODPHV(ii)
     MSFlexGrid1.TextMatrix(ii, 8) = CODPMP(ii)
   Next
  End If
