...
VERSION 5.00
Begin VB.Form frmBWSC 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Data for BWSC Pipe"
   ClientHeight    =   2850
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5475
   ...
   Begin VB.Label Label2 
      Caption         =   "Steel Cylinder Thickness (mm) "
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
...
Private Sub cmdOK_Click()
 ...
 CORET(NP) = Val(txtSCT.Text)
 COATT(NP) = Val(txtCT.Text)
 STBYV(NP) = Val(txtSBV.Text)
 PWTHICK = CORET(NP) + (1 - STBYV(NP) / 100) * (COATT(NP) / 12) + (STBYV(NP) / 100) * (COATT(NP))
 If PWTHICK <= 0 Then
