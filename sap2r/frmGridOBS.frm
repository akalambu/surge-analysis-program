...
Begin VB.Form frmGridOBS 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Local Obstruction Details"
   ClientHeight    =   3750
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5685
   ...
   Begin VB.Frame frmINRVs 
      Caption         =   "Enter the Local Obstruction Details"
      BeginProperty Font 
         Name            =   "Comic Sans MS"
         Size            =   12
         Charset         =   0
...
Dim jj As Integer
 ...
 For ii = 1 To NOBS
   HGL1OB(ii) = MSFlexGrid1.TextMatrix(ii, 2)
   HGL2OB(ii) = MSFlexGrid1.TextMatrix(ii, 3)
   QOOBS(ii) = PDC(IUSP(MSFlexGrid1.TextMatrix(ii, 1), 1))
   NPUSOB(ii) = IUSP(MSFlexGrid1.TextMatrix(ii, 1), 1)
   IOBS(MSFlexGrid1.TextMatrix(ii, 1)) = ii
 Next
...
'set array
  ...
  If Not OpenFile = "" Then
   For ii = 1 To MSFlexGrid1.Rows - 1
       MSFlexGrid1.TextMatrix(ii, 2) = HGL1OB(ii)
       MSFlexGrid1.TextMatrix(ii, 3) = HGL2OB(ii)
   Next
  End If
