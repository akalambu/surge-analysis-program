  ...
  For ii = 0 To (NACVD - 1) Step 1
     For jj = 0 To 2 Step 1
        If Not IsNumeric(MSFlexGrid1.TextMatrix(ii + 1, jj + 1)) Then
         ...
         Exit For
         End If
        AC_Data(ii, jj + 1) = MSFlexGrid1.TextMatrix(ii + 1, jj + 1)
        Next
     If flag = 1 Then
      Exit For
     End If
 ...
 If NACVD > 0 And Not OpenFile = "" Then
   For i = 1 To MSFlexGrid1.Rows - 1
     For j = 1 To MSFlexGrid1.Cols - 1
       MSFlexGrid1.TextMatrix(i, j) = AC_Data(i - 1, j)
     Next
   Next
 End If
