...
Begin VB.Form frmgridOWSTA 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Data for OST "
   ClientHeight    =   3870
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   9015
   ...
   Begin VB.TextBox txtOST 
      ...
      Width           =   855
   End
   Begin VB.Frame frmOWST 
      Caption         =   "Enter the Data Required for the OST"
      BeginProperty Font 
         Name            =   "Comic Sans MS"
         Size            =   12
         Charset         =   0
   ...
   Begin VB.Label Label1 
      Caption         =   " Number of One Way Surge Tanks "
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmgridOWSTA"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Const TOTALCOLUMNS = 7  'Zero based
...
flag = 0
NOSTD = Val(txtOST.Text)

If NOSTD > 0 Then
  For ii = 0 To (NOSTD - 1) Step 1
     For jj = 0 To 5 Step 1
        If Not IsNumeric(MSFlexGrid1.TextMatrix(ii + 1, jj + 1)) Then
         MsgBox "Data is not Complete, Please Check It !!"
         flag = 1
         Exit For
         End If
        OST_Data(ii, jj + 1) = MSFlexGrid1.TextMatrix(ii + 1, jj + 1)
     Next
     If flag = 1 Then
      Exit For
     End If
...
Private Sub Form_Load()
   ...
   myArray = Array("OST No.", "Chainage   (m)", "Ground Level (RL, m)", "Staging Height (m)", "Diameter   (m)", "Storage Depth (m)", "Connecting Pipe Dia (mm)")
  
   If NOSTD > 0 Then
    MSFlexGrid1.Rows = NOSTD + 1
    txtOST = NOSTD
   Else
    MSFlexGrid1.Rows = 2
   End If
   ...
   If NOSTD > 0 And Not OpenFile = "" Then
   For i = 1 To MSFlexGrid1.Rows - 1
     For j = 1 To MSFlexGrid1.Cols - 1
       MSFlexGrid1.TextMatrix(i, j) = OST_Data(i - 1, j)
     Next
   Next
   End If
   'highlight 1st row
...
Else
  ...
  MSFlexGrid1.Rows = txtOST.Text + 1
  For i = 1 To MSFlexGrid1.Rows - 1
    MSFlexGrid1.TextMatrix(i, 0) = i
  Next
End If
