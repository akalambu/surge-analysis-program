 ...
 For ii = 0 To (NDPCVD - 1) Step 1
     For jj = 0 To 1 Step 1
        If Not IsNumeric(MSFlexGrid1.TextMatrix(ii + 1, jj + 1)) Then
         ...
         Exit For
         End If
        DPC_Data(ii, jj + 1) = MSFlexGrid1.TextMatrix(ii + 1, jj + 1)
        Next
     If flag = 1 Then
      Exit For
     End If
  Next
Else
frmProtA.chkDualPl.Value = 0
frmProtA.chkZeroV.Enabled = True
End If
If flag = 0 Then
 Me.Hide
...
Private Sub Form_Load()
   ...
   'set array
    myArray = Array("DPCV No.", "Chainage   (m)", "Bypass Size of DPCV (mm)")
  ...
  If NDPCVD > 0 And Not OpenFile = "" Then
   For i = 1 To MSFlexGrid1.Rows - 1
     For j = 1 To MSFlexGrid1.Cols - 1
       MSFlexGrid1.TextMatrix(i, j) = DPC_Data(i - 1, j)
     Next
   Next
  End If
   'highlight 1st row
